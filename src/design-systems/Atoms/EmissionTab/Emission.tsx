...
export const Emission: React.FC = () => {
  ...
  const headData = ["", "My Votes", "Votes", "Estimated ALTO Emissions"];
  return (
    <div>
      <div className="flex justify-between px-6">
        <div className="flex">
          {EmissionTab.map((item, key) => (
            <button
              className={`p-[11px] hover:bg-gray04 rounded text-sm text-black54`}
              key={key}
            >
              {item.label}
            </button>
        ...
        <input
          className="p-2 border-[1px] border-primary rounded-xs bg-blue245"
          placeholder="Search..."
        />
      </div>
      ...
      <div>
        ...
        <div className="flex flex-col items-center w-full mt-8 mb-3">
          <Typography className="text-[18px] text-lightBlack">
            Lock some ALTO to start voting
          </Typography>
          <div className="flex gap-4 mt-3">
            <Button className="bg-primary py-[6px] px-[16px] text-white font-semibold rounded">
              Cast Votes
            </Button>
            <Button className="bg-primary py-[6px] px-[16px] text-white font-semibold rounded">
              Register Vote Weight
            </Button>
          </div>
        </div>
      </div>
