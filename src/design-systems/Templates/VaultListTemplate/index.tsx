      ...
      <div className="w-[576px] flex items-center justify-between rounded-full bg-[#F0F0F0]">
        {tabs.map(({ label, name }) => (
          <button key={name} onClick={() => setSaleType(name)}>
            <div
              className={`enabled:active:bg-brand-hover cursor-pointer bg-white rounded-full px-24 w-[335px] py-2 font-poppins text-small font-semibold leading-[14px] hover:bg-gradient-to-t disabled:opacity-30 ${
                name === saleType
                  ? "border-[3px] border-blue"
                  : "border-[3px] border-blue opacity-[0.5] z-0"
              }`}
            >
      ...
      <div className="w-full bg-white shadow-lg rounded-[12px] max-w-[1152px] mt-5">
        <div className="flex flex-col gap-3 items-start p-4">
          ...
          <Typography className="text-body font-normal text-left text-darkBlue">
            Vaults with the lowest collateral ratio are the first to be redeemed
            against.
            <span className="text-blue225 border-b border-blue225">
              Learn more.
            </span>{" "}
            Additionally vaults below 150% collateral ratio are at risk of
            liquidation if the system enters recovery mode.{" "}
            <span className="text-blue225 border-b border-blue225">
              Learn more.
            </span>
          </Typography>
